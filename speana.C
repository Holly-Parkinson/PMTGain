...
void speana() {
  ...
  bool all_events = false; //do all events or just the selected one specified by eventid
...
if (!all_events) {
  ...
  cout << thresh*(highestval-mean) << endl;
  ...
  for (int i=0; i<np; i++) { //loop through all peak positions
    if (wvf_pt[i] >= (wvf_tlo + sperange_tlo) && wvf_pt[i] <= (wvf_tlo + sperange_thi)) {
      ...
      cout << "SPE x position = " << wvf_pt[i] << endl;
      j++;
    } 
  }
  ...
  //}
 cout << baseline << " = baseline" << endl; //above commented out for test 
...
if (do_avgspe) {
  for (int i=0; i<nspe; i++) { //iterate through the found spe positions
    ...
    cout << "bin associated with peak time " << peakbin << endl;
...
if (do_amp) {
  for (int i=0; i<nspe; i++) { //iterate through the found spe positions
    ...
    cout << peakheight << " = peak height" << endl;
